...
const relativeDate = (iso, t) => {
  ...
  return t('Yesterday').startsWith('A') ? `${diff} ${t('days ago')}` : `${t('Hace')} ${diff} ${t('días')}`;
};
...
export const FinanceWallets = () => {
  ...
  const activeProjects = useSelector((s) =>
    (s.projects?.projects || []).filter((project) =>
      ['active', 'in-progress', 'planning', 'maintenance'].includes(project.status)
    )
  );
  ...
  const [conversionDirection, setConversionDirection] = useState('USD_TO_MXN');
  ...
  const handleIncomeUSD = (e) => {
    ...
    dispatch(addIncomeUSD({
      id: `inc-usd-${Date.now()}`,
      amount: amt,
      description: form.description || t('Ingreso USD'),
  ...
  const handleIncomeMXN = (e) => {
    ...
    dispatch(addIncomeMXN({
      id: `inc-mxn-${Date.now()}`,
      amount: amt,
      description: form.description || t('Ingreso MXN'),
      category: form.category || 'income',
      date: new Date(form.date).toISOString(),
    }));
    resetForm();
  ...
  const handleConversion = (e) => {
    ...
    if (conversionDirection === 'USD_TO_MXN') {
  ...
  const handleExpense = (e) => {
    ...
    dispatch(
      registerExpense({
        ...
        projectId: form.linkedProject || null,
        description: form.description || t('Gasto'),
        date: new Date(form.date).toISOString(),
      })
    );
    resetForm();
  ...
  const handleTransferToSavings = (e) => {
    ...
    dispatch(transferToSavings({
      ...
      amount: amt,
      currency: form.currency,
      description: form.description || t('Transferencia a ahorros'),
      date: new Date(form.date).toISOString(),
    }));
    resetForm();
    setActiveForm(null);
  ...
  const handleWithdrawFromSavings = (e) => {
    ...
    dispatch(withdrawFromSavings({
      ...
      amount: amt,
      currency: form.currency,
      description: form.description || t('Retiro de ahorros'),
      date: new Date(form.date).toISOString(),
    }));
    resetForm();
    setActiveForm(null);
  ...
  const filteredTx = useMemo(() => {
    ...
    if (txFilter === FILTER_CONVERSION)
      return transactions.filter((t) => t.type === 'conversion' || t.type === 'conversion_mxn_to_usd');
    return transactions;
  ...
  const txColor = (type) => {
    ...
    if (type === 'conversion' || type === 'conversion_mxn_to_usd') return 'var(--accent-cyan, #1ec9ff)';
  ...
  const txLabel = (tx) => {
    ...
    if (tx.type === 'conversion_mxn_to_usd')
      return `-$${fmtMXN(tx.amountMXN)} MXN → +$${fmtUSD(tx.amountUSD)} USD (tasa $${tx.rate?.toFixed(2)})`;
    return '';
  };
    ...
    <div className="wallets-container">
      ...
      {activeForm === 'income_usd' && (
        <form className="wallets-form" onSubmit={handleIncomeUSD}>
          <h3>💵 {t('Ingreso USD')}</h3>
          <label>{t('Descripción')}
            <input value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder={t('Ej: Cliente XYZ')} />
          </label>
          <label>{t('Monto USD')}
            <input type="number" min="0.01" step="0.01" required value={form.amount} onChange={(e) => setForm((p) => ({ ...p, amount: e.target.value }))} placeholder={t('0.00')} />
          </label>
          <label>{t('Categoría')}
      ...
      {activeForm === 'income_mxn' && (
        <form className="wallets-form" onSubmit={handleIncomeMXN}>
          <h3>💴 {t('Ingreso MXN')}</h3>
          <label>{t('Descripción')}
            <input value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder={t('Ej: Venta local')} />
          </label>
          <label>{t('Monto MXN')}
            <input type="number" min="0.01" step="0.01" required value={form.amount} onChange={(e) => setForm((p) => ({ ...p, amount: e.target.value }))} placeholder={t('0.00')} />
          </label>
      ...
      {activeForm === 'conversion' && (
        <form className="wallets-form wallets-form--conversion" onSubmit={handleConversion}>
          <h3>↔ {t('Conversión de divisas')}</h3>
          <div className="wallets-conversion-direction" role="group" aria-label={t('Direction')}>
            <button
              type="button"
              className={`wallets-conversion-direction-btn${conversionDirection === 'USD_TO_MXN' ? ' active' : ''}`}
              onClick={() => setConversionDirection('USD_TO_MXN')}
            >
              {t('USD to MXN')}
            </button>
            <button
              type="button"
              className={`wallets-conversion-direction-btn${conversionDirection === 'MXN_TO_USD' ? ' active' : ''}`}
              onClick={() => setConversionDirection('MXN_TO_USD')}
            >
              {t('MXN to USD')}
            </button>
          </div>
          <p className="wallets-form-hint">{t('¿Cuánto convertiste y a qué tasa?')}</p>
          <label>{conversionDirection === 'USD_TO_MXN' ? t('Monto USD que convertiste') : t('Monto USD que recibiste')}
            <input type="number" min="0.01" step="0.01" required value={form.amountUSD} onChange={(e) => setForm((p) => ({ ...p, amountUSD: e.target.value }))} placeholder={t('0.00')} />
          </label>
          <label>{conversionDirection === 'USD_TO_MXN' ? t('Monto MXN que recibiste') : t('Monto MXN que convertiste')}
            <input type="number" min="0.01" step="0.01" required value={form.amountMXN} onChange={(e) => setForm((p) => ({ ...p, amountMXN: e.target.value }))} placeholder={t('0.00')} />
          </label>
          {conversionRate !== null && (
            <div className="wallets-rate-preview">
              {conversionDirection === 'USD_TO_MXN'
                ? (<>{t('Convirtiendo')} <strong>${fmtUSD(form.amountUSD)} USD</strong> → <strong>${fmtMXN(form.amountMXN)} MXN</strong></>)
                : (<>{t('Convirtiendo')} <strong>${fmtMXN(form.amountMXN)} MXN</strong> → <strong>${fmtUSD(form.amountUSD)} USD</strong></>)}
              <br />
              {t('Tasa registrada')}: <strong>${conversionRate.toFixed(4)} MXN {t('por')} USD</strong>
            </div>
          )}
          <label>{t('Descripción')} ({t('opcional')})
            <input value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder={t('Ej: Casa de cambio OXXO')} />
          </label>
          <label>{t('Fecha')}
            <input type="date" value={form.date} onChange={(e) => setForm((p) => ({ ...p, date: e.target.value }))} />
          </label>
      ...
      {activeForm === 'expense' && (
        <form className="wallets-form" onSubmit={handleExpense}>
          ...
          <label>{t('Descripción')}
            <input required value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder={t('Ej: Supermercado')} />
          </label>
          <label>{t('Monto')}
            <input type="number" min="0.01" step="0.01" required value={form.amount} onChange={(e) => setForm((p) => ({ ...p, amount: e.target.value }))} placeholder={t('0.00')} />
          </label>
          ...
          {activeProjects.length > 0 && (
            <label>{t('Proyecto vinculado')}
              <select value={form.linkedProject} onChange={(e) => setForm((p) => ({ ...p, linkedProject: e.target.value }))}>
                <option value="">{t('Sin proyecto')}</option>
                {activeProjects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}
              </select>
            </label>
          )}
          <label>{t('Fecha del gasto')}
            <input type="date" value={form.date} onChange={(e) => setForm((p) => ({ ...p, date: e.target.value }))} />
          </label>
          <p className="wallets-form-hint">{t('Puedes registrar gastos de días anteriores para mantener historial correcto.')}</p>
          <div className="wallets-form-actions">
            <button type="button" onClick={() => setActiveForm(null)}>{t('Cancelar')}</button>
        ...
        <form className="wallets-form" onSubmit={handleTransferToSavings}>
          ...
          <label>{t('Monto')} *
            <input type="number" min="0.01" step="0.01" required value={form.amount}
              onChange={(e) => setForm((p) => ({ ...p, amount: e.target.value }))} placeholder={t('0.00')} />
          </label>
          <label>{t('Divisa')}
      ...
      <section className="wallets-history">
        ...
        {filteredTx.length === 0 ? (
          <EmptyState
            ...
            title={t('No tengo transacciones registradas.')}
            description={
              txFilter === FILTER_SAVINGS
                ? t('Si mueves dinero a ahorros, empiezo a construir ese historial.')
                : t('Registra un ingreso o un gasto y te muestro el historial aqui.')
            }
            action={{
              label: txFilter === FILTER_SAVINGS ? t('Mover a ahorros') : t('Registrar ingreso MXN'),
              icon: '+',
            ...
            {filteredTx.map((tx) => (
              <li key={tx.id} className="wallets-tx-item">
                ...
                <span className="wallets-tx-amount" style={{ color: tx.type === 'deposit' ? 'var(--accent-cyan,#1ec9ff)' : 'var(--color-warning,#f59e0b)' }}>
                  {tx.type === 'deposit'
                    ? `→ ${t('Ahorros')} +$${tx.currency === 'USD' ? fmtUSD(tx.amount) : fmtMXN(tx.amount)} ${tx.currency}`
                    : `← ${t('De Ahorros')} +$${tx.currency === 'USD' ? fmtUSD(tx.amount) : fmtMXN(tx.amount)} ${tx.currency}`}
                </span>
                <button

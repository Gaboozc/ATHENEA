...
export const Projects = () => {
  ...
  const currentUserId = useSelector(
    (state) => state.auth?.user?.id || state.user?.id || null
  );
  ...
  const handleCreateSubmit = (event) => {
    ...
    const activeTemplate = PROJECT_TEMPLATES.find((tpl) => tpl.id === selectedTemplate) || null;
    const payload = {
      id: projectId,
      name: formData.name,
    ...
    if (activeTemplate?.tasks?.length) {
      const startDateBase = formData.startDate ? new Date(`${formData.startDate}T09:00:00`) : new Date();
      const nowIso = new Date().toISOString();

      activeTemplate.tasks.forEach((templateTask, index) => {
        const dueDate = new Date(startDateBase);
        dueDate.setDate(dueDate.getDate() + Number(templateTask.daysFromStart || 0));

        addTask({
          id: `task-${projectId}-${index}`,
          title: templateTask.title,
          description: `${t('Task auto-created from template')}: ${activeTemplate.name}`,
          status: 'Pendiente',
          level: templateTask.priority === 'critica'
            ? 'Critical'
            : templateTask.priority === 'alta'
            ? 'High Velocity'
            : templateTask.priority === 'media'
            ? 'Steady Flow'
            : 'Backlog',
          projectId,
      ...
      showToast(
        `${activeTemplate.tasks.length} ${t('tasks created from template')}`,
        'success',
        2500,
        activeTemplate.icon || '✅'
      );
          ...
          <div className="project-create-card" onClick={(event) => event.stopPropagation()}>
            ...
            {showTemplates && (
              <div className="template-selector">
                <h3>{t('Choose a project template')}</h3>
                <p>{t('Start faster with predefined freelance workflows.')}</p>
                <div className="template-grid">
                  {PROJECT_TEMPLATES.map((template) => (
                    ...
                    >
                      ...
                      <div className="template-duration">{template.estimatedDuration}</div>
                      <div className="template-tasks-count">{template.tasks.length} {t('tasks')}</div>
                    </button>
                  ))}
                </div>
                <button
            ...
            <form onSubmit={handleCreateSubmit} className="project-create-form">
              {selectedTemplate && (
                <div className="template-selected-indicator">
                  {PROJECT_TEMPLATES.find((tpl) => tpl.id === selectedTemplate)?.icon}{' '}
                  {t('Template selected')}:{' '}
                  {PROJECT_TEMPLATES.find((tpl) => tpl.id === selectedTemplate)?.name}
                </div>
              )}
